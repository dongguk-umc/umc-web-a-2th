...
// 로컬스토리지에 저장할 정보를 만드는 함수 생성
function setItem (){
    localStorage.setItem('comments', JSON.stringify(comments)); // comments는 배열이므로 문자열화 하여 저장
}

function displayHistory (){

    const savedComments = JSON.parse(localStorage.getItem('comments')); // 배열화하여 정보를 가져옴

    savedComments.map(comment => {
        const newCommentItem = commentItemTemplate(comment); // html코드가 담긴 아이템 생성
        comments.push(comment); // comment도 계속 갱신 해줘야 하므로 push
        $commentList.insertAdjacentHTML('afterbegin', newCommentItem); // html 끼워 넣기
    });
}
